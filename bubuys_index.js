...
function fileManagerHtml(startPath) {
  ...
  html += 'function _audioNext(){if(_audioPlaylist.length<=1)return;if(_loopMode===0&&_audioCurIdx>=_audioPlaylist.length-1){return}_audioCurIdx=(_audioCurIdx+1)%_audioPlaylist.length;playMedia(_audioPlaylist[_audioCurIdx])}';
  html += 'function _audioPrev(){if(_audioPlaylist.length<=1)return;_audioCurIdx=(_audioCurIdx-1+_audioPlaylist.length)%_audioPlaylist.length;playMedia(_audioPlaylist[_audioCurIdx])}';
  html += 'function _renderPlaylist(){var _pl=wrap.querySelector("#fmPlList");if(!_pl)return;var _h="";for(var _i=0;_i<_audioPlaylist.length;_i++){var _it=window._items[_audioPlaylist[_i]];var _cls=_i===_audioCurIdx?"fm-pl-item active":"fm-pl-item";_h+="<div class=\\\""+_cls+"\\\" data-idx=\\\""+_i+"\\\"><span class=\\\"fm-pl-idx\\\">"+(_i+1)+"</span><span class=\\\"fm-pl-name\\\">"+_it.name+"</span></div>"}_pl.innerHTML=_h;var _items=_pl.querySelectorAll(".fm-pl-item");for(var _j=0;_j<_items.length;_j++){_items[_j].onclick=function(){var _di=parseInt(this.dataset.idx);_audioCurIdx=_di;playMedia(_audioPlaylist[_di]);_closePlaylist()}}}';
