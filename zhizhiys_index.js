...
function parseCards(html) {
  ...
  while ((m = reg.exec(html))) {
    ...
    cards.push({ title, url, img: proxyImg, tag, desc: '', meta: '', actors: '', intro: '' });
  }
  return cards;
}
...
function parseTypeFilters(html) {
  const opts = [];
  const m = html.match(/module-item-title">(?:类型|剧情)[\s\S]*?<div class="module-item-box">([\s\S]*?)<\/div>/);
  if (!m) return opts;
  ...
  for (const lk of links) {
    ...
    // 新站格式: /show/cid---genre--------.html, genre 在第4段
    const parts = href.replace('/show/','').replace('.html','').split('-');
    const slug = parts[3] || parts[0];
...
// ========== 解析播放线路和集数 ==========
// 解析 MacCMS 播放页 player_aaaa + 集数列表
function parsePlaySources(html) {
  ...
  // 解析集数按钮
  const epReg = /<a[^>]*href="(\/vodplay\/[0-9]+-[0-9]+-([0-9]+)\.html)"[^>]*>([^<]*)<\/a>/g;
  const eps = [];
  let m;
  while ((m = epReg.exec(html))) {
    eps.push({ url: m[1], title: strip(m[3]) || ('第' + m[2] + '集') });
  }
  if (eps.length) sources.push({ name: '默认线路', episodes: eps });
  // 附带 player_aaaa 信息
  if (playerData) {
    sources._playerData = playerData;
  }
  return sources;
}
...
function handleHomeApi(res) {
  fetchPage(SITE + '/', (err, html) => {
    ...
    try {
      let lunbos = [];
      const sections = [];
      // zzoc.cc: 按 myui-vodbox 分块
      const blocks = html.split('<div class="myui-vodbox">');
      for (let i = 1; i < blocks.length; i++) {
        const block = blocks[i];
        // 区块标题: <div class="title...">TITLE</div>
...
function openVod(it){
  ...
  if(!/^https?:/.test(item.url)&&item.url.indexOf('duanju://')!==0)item.url='https://www.zzoc.cc'+item.url;
...
function openVod(it){var item=Object.assign({},it);if(!/^https?:/.test(item.url)&&item.url.indexOf('duanju://')!==0)item.url='https://www.zzoc.cc'+item.url;try{parent.postMessage({type:'dsjDetail',item:item},'*')}catch(e){location.href=item.url}}
  ...
  if(IS_DUANJU){
    if(playSources.length){
      ...
      if(src&&src.episodes&&src.episodes.length){
        ...
        try{var _t=document.querySelector('.info .t'),_b=document.querySelector('.bg img'),_hi=document.getElementById('_hi');sessionStorage.setItem('youzi_tmdb_meta_'+VOD_URL,JSON.stringify({title:_t?_t.textContent:ep.title,backdrop:_b?_b.src:'',img:_hi?_hi.dataset.img:''}))}catch(e){}
...
function renderEps(){
  ...
  showEps.forEach(function(ep){
    ...
    d.onclick=function(){
      ...
      renderEps();
      window.location.href='/player?url='+encodeURIComponent(u)+'&title='+encodeURIComponent(ep.title)+'&vod='+encodeURIComponent(VOD_URL);
      try{var _bgi=document.querySelector('.bg img'),_hi=document.getElementById('_hi');sessionStorage.setItem('youzi_tmdb_bg_'+VOD_URL,(_bgi?_bgi.src:''));sessionStorage.setItem('youzi_tmdb_meta_'+VOD_URL,JSON.stringify({title:document.querySelector('.info .t')?document.querySelector('.info .t').textContent:ep.title,backdrop:_bgi?_bgi.src:'',img:_hi?_hi.dataset.img:''}))}catch(e2){}
    };
    grid.appendChild(d)
  });
...
window.addEventListener('pagehide',function(){try{parent.postMessage({type:'dsjShowChrome'},'*')}catch(e){}});
window.addEventListener('beforeunload',function(){try{parent.postMessage({type:'dsjShowChrome'},'*')}catch(e){}});
...
function _autoPlay(){video.muted=true;video.play().then(function(){setTimeout(function(){video.muted=false},100)}).catch(function(){});_restoreProgress()}
function _restoreProgress(){if(!VOD_URL)return;var _hti=VOD_URL.replace(/[^a-zA-Z0-9]/g,'_');fetch('/his-list').then(function(r){return r.json()}).then(function(j){if(!j.ok||!j.items)return;var h=null;for(var i=0;i<j.items.length;i++){if(j.items[i].id===_hti){h=j.items[i];break}}if(!h||!h.progress||!h.duration)return;if(h.progress<h.duration-5&&h.progress>5){video.currentTime=h.progress}}).catch(function(){})}
...
(function(){var _c=null;try{_c=JSON.parse(sessionStorage.getItem('youzi_ep_'+VOD_URL)||'null')}catch(e){}if(_c&&_c.sources&&_c.sources.length){pSources=_c.sources;pCurSrc=_matchSrcIdx();renderSrcTabs();renderEpList()}else{loadEpisodes()}})();
<\/script></body></html>`;
}
...
const server = http.createServer((req, res) => {
  ...
  if (pathname === '/api') {
    ...
    // 新站格式: /show/{cid}-{地区}-{排序}-{剧情}-{语言}-{字母}-{…}-{页码}-{…}.html
    // filter 放在第4段（剧情位置），cid 保持在第1段
    // zzoc.cc: /vodtype/{id}.html
    const cidMap = { 'dianying': '1', '2': '2', 'zongyi': '3', 'dongman': '4' };
    const typeId = cidMap[cid] || cid;
    const url = page > 1 ? `${SITE}/vodtype/${typeId}-${page}.html` : `${SITE}/vodtype/${typeId}.html`;
  ...
  if (pathname === '/search-api') {
    ...
    const url = `${SITE}/vodsearch/${encodeURIComponent(wd)}-------------.html`;
    return fetchPage(url, (err, html) => {
      if (err) return send(res, 200, JSON.stringify({ok:false,error:err.message}));
