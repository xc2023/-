...
function row(it){var d=document.createElement('div');d.className='row';var tagHtml=it.tag?'<span class="sptext">'+it.tag+'</span>':'';var seed=Math.floor(Math.random()*1000);d.innerHTML='<div class="sposter"><img loading="lazy" src="'+(it.img||'')+'" onerror="this.src=\'https://picsum.photos/seed/'+seed+'/300/400\'">'+tagHtml+'</div><div class="sinfo"><div class="sname">'+it.title+'</div>'+(it.desc?'<div class="smeta" style="-webkit-line-clamp:5">'+it.desc+'</div>':'')+'</div>';d.onclick=function(){openVod(it)};return d}
