...
function row(it){var d=document.createElement('div');d.className='row';var tagHtml=it.tag?'<span class="sptext">'+it.tag+'</span>':'';var seed=Math.floor(Math.random()*1000);var fallback='https://picsum.photos/seed/'+seed+'/300/400';d.innerHTML='<div class="sposter"><img loading="lazy" src="'+(it.img||'')+'" onerror="this.src=\''+fallback+'\'">'+tagHtml+'</div><div class="sinfo"><div class="sname">'+it.title+'</div>'+(it.desc?'<div class="smeta" style="-webkit-line-clamp:5">'+it.desc+'</div>':'')+'</div>';d.onclick=function(){openVod(it)};return d}
...
const server = http.createServer((req, res) => {
  ...
  if (path === '/search-api') {
    ...
    const url = `${SITE}/vod/search/wd/${encodeURIComponent(wd)}${pg}.html`;
    return fetchPage(url, (err, html) => {
      if (err) return send(res, 200, JSON.stringify({ok:false,error:err.message}));
      const items = parseCards(html);
      send(res, 200, JSON.stringify({ok:true, items}), 'application/json');
